...
STDMETHODIMP CConnect::OnConnection(IDispatch *pApplication, ext_ConnectMode ConnectMode, IDispatch *pAddInInst, SAFEARRAY ** /*自定义*/ )
{
	...
	if(ConnectMode == 5) //5 == ext_cm_UISetup
	{
		...
		hr = (pCommands2->AddNamedCommand2(m_pAddInInstance, CComBSTR("VisualFC"), CComBSTR("VisualFC"), CComBSTR("Executes VisualFC ClassWizard"), VARIANT_TRUE, CComVariant(59), NULL, EnvDTE::vsCommandStatusSupported+EnvDTE::vsCommandStatusEnabled, vsCommandStylePictAndText, vsCommandControlTypeButton, &pCreatedCommand)) && (pCreatedCommand);
		if (SUCCEEDED(hr))
		{
			//在“工具”菜单栏上添加一个按钮。
...
STDMETHODIMP CConnect::QueryStatus(BSTR bstrCmdName, EnvDTE::vsCommandStatusTextWanted NeededText, EnvDTE::vsCommandStatus *pStatusOption, VARIANT *pvarCommandText)
{
  if(NeededText == EnvDTE::vsCommandStatusTextWantedNone)
	{
	  if(!_wcsicmp(bstrCmdName, L"FreeCodeNET.Connect.VisualFC"))
	  {
...
bool CConnect::ExecVFCWizard()
{
	if (m_fc.pvfc)
	{
		...
		CComBSTR name;
		pProj->get_Name(&name);
		m_fc.pvfc->load_project(GetActiveWindow(),(CString)name,"",false);
	}
	return true;
}
...
bool CConnect::add_project_file(const char * file)
{
	EnvDTE::ProjectPtr pProj;
	HRESULT hr = S_FALSE;
	if (GetActiveProject(pProj) == S_FALSE || pProj == NULL)
...
const char * CConnect::get_project_path()
{
	EnvDTE::ProjectPtr pProj;
	if (GetActiveProject(pProj) == S_FALSE || pProj == NULL)
	{
...
const char * CConnect::get_project_name()
{
	EnvDTE::ProjectPtr pProj;
	if (GetActiveProject(pProj) == S_FALSE || pProj == NULL)
	{
