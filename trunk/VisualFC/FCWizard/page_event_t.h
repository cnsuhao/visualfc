...
struct page_event_peer
{
	...
	virtual bool init_event(HWND hWndTree, HWND hWndList, CAtlArray<HTREEITEM> * pItems) = 0;
...
public:
	...
	BEGIN_MSG_MAP(page_event_template)
		COMMAND_ID_HANDLER(IDC_KIND_BTN, OnKindBtn)
		COMMAND_ID_HANDLER(IDC_LIST_BTN, OnListBtn)
	...
	void OnInit()
	{
		...
		if (m_peer)
		{
			if (!m_peer->init_event(m_evTree, m_evList,&m_tree_items))
			{
