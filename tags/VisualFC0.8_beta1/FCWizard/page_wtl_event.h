...
struct page_wtl_event_peer : page_event_peer
{
	...
	virtual bool init_event(HWND hWndTree, HWND hWndList, CAtlArray<HTREEITEM> * pItems)
	{
		...
		if (!m_kind.loadex(GetModulePath(_Module.m_hInst)+_T("\\wtlkind.ini")))
		{
			return false;
		}
		CTreeViewCtrl tree(hWndTree);
		for (size_t i = 0; i < m_kind.m_items.GetCount(); i++)
		{
			wtl_kind & kind = m_kind.m_items[i];
			...
			for (int j = 0; j < kind.msgs.GetCount(); j++)
			{
				...
				for (int k = 0; k < m_wev.m_items.GetCount(); k++)
				{
					wtl_event & ev = m_wev.m_items[k];
